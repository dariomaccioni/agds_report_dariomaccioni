...
results <- list() # Defining a list which stores the whole results
...
for (predictor in c("none", "siteid", "TIMESTAMP", "TA_F", "SW_IN_F", "LW_IN_F", "VPD_F", 
  ...
  formula_01 <- as.formula(formula_string) # Creating a formula object
  
  model <- lm(formula_01, data = half_hourly_fluxes_fixed) # Fit the linear model
...
results_final <- as.data.frame(do.call(rbind, results)) # Converting the list into a dataframe

colnames(results_final) <- c("formula", "AIC", "r_squared", "adj_r_squared") # renaming the columns for better understanding

results_final$AIC <- as.numeric(results_final$AIC) # Converting AIC to numeric

results_final <- results_final[order(results_final$AIC), ] # Sorting the data frame by AIC value in ascending order

print(results_final) # printing the results
```
