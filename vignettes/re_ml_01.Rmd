...
k_values <- c(1:30)
mae_values <- sapply(k_values, get_mae)

# Visualize MAE as a function of k
df <- data.frame(k = k_values, MAE = mae_values)
ggplot(df, aes(x = k, y = MAE)) + 
  geom_line() + 
  geom_point() + 
  xlab("k") + 
  ylab("MAE") +
  geom_hline(yintercept = test_mae, color = "red") +
  annotate("text", x = best_k, y = test_mae + 0.5, label = "Test MAE", color = "red")

# Find value of k that minimizes MAE
best_k <- k_values[which.min(mae_values)]
cat("The best value of k is", best_k)

# Evaluate KNN model with best_k on test set
test_mae <- get_mae(best_k)
cat("The MAE on the test set is", test_mae)
