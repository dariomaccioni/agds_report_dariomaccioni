...
eval_model <- function(mod, df_train, df_test){
  ...
  # getting the tables of the metrics
...
get_mae <- function(k) {
  ...
  new_data_01 <- drop_na(half_hourly_fluxes_test)
  pred <- predict(model_knn, newdata = new_data_01)
  obs <- new_data_01$GPP_NT_VUT_REF
  mae <- caret::MAE(pred, obs)
  
  # Evaluate model on training set
  prediction_train <- predict(model_knn, newdata = drop_na(half_hourly_fluxes_train))
  observed_train <- drop_na(half_hourly_fluxes_train)$GPP_NT_VUT_REF
  mae_train <- caret::MAE(prediction_train, observed_train)
